module.exports = {
  ...
  accessControl: {
    ...
    note1:'1.调度规则是用于限制已下发人员的通行时间，请确保人脸信息已全部下发至设备再生成下发任务！',
    note2:'2.规则下发顺序是：工作分类->部门->员工。如出现重复，部门规则将覆盖工作分类规则，员工规则覆盖前两个。',
    note3:'3.对于部门下发规则，不会查找该部门的上下级部门的从属关系，即某个员工如果属于A部门，那么当添加部门下发规则时，必须选择A部门，选择A部门的上级不会下发该员工。',
    note4:'4.调度规则为不能通行的人员，刷脸比对时相机将播报‘非调度时间’而不是‘未授权’。',
    note5:'5.如果默认调度规则为‘不得进出’，系统将给未添加规则的人员设置为默认调度类别，即全天不能通行。',
    ...
    fullAccess: '不限制进出',
    defaultAccess: '默认规则:',
    ruleTips:'定义可通行时间段，对应人员不在时间段内不可通行',
